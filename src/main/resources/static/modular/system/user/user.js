...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/user/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            var ajax = new $ax(Feng.ctxPath + "/user/delete/"+userId,"DELETE", function () {
                Feng.success("删除成功!");
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        ...
        var ajax = new $ax(Feng.ctxPath + "/user/freeze","PUT", function (data) {
            Feng.success("冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/user/unfreeze","PUT", function (data) {
            Feng.success("解除冻结成功!");
            MgrUser.table.refresh();
        }, function (data) {
            Feng.error("解除冻结失败!");
...
MgrUser.resetPwd = function () {
    if (this.check()) {
        ...
        parent.layer.confirm('是否重置密码为111111？', {
            ...
            var ajax = new $ax(Feng.ctxPath + "/user/reset","PUT", function (data) {
                Feng.success("重置密码成功!");
            }, function (data) {
                Feng.error("重置密码失败!");
            });
...
$(function () {
    ...
    var table = new BSTable("managerTable", "/user/list", defaultColunms,"GET");
