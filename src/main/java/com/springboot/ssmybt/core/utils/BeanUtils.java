...
import com.springboot.ssmybt.core.exception.ToolBoxException;
/**
 * Bean工具类x
 * @author liuc
 *
        ...
        if (isToCamelCase) {
            ...
            for (Entry<?, ?> entry : map.entrySet()) {
                ...
                if (null != key && key instanceof String) {
                    ...
                    map2.put(StringUtils.toCamelCase(keyStr), entry.getValue());
                } else {
        ...
        return fillBean(bean, new ValueProvider() {
            ...
            public Object value(String name) {
                ...
                if (StringUtils.isEmpty(value)) {
                    // 使用类名前缀尝试查找值
                    value = request.getParameter(beanName + StringUtils.DOT + name);
                    if (StringUtils.isEmpty(value)) {
                        // 此处取得的值为空时跳过，包括null和""
                        value = null;
                    }
        ...
        try {
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                ...
                if (false == key.equals("class")) {
                    ...
                    if (null != value) {
                        map.put(isToUnderlineCase ? StringUtils.toUnderlineCase(key) : key, value);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public static void copyProperties(Object source, Object target, CopyOptions copyOptions) {
        ...
        if (copyOptions.editable != null) {
            ...
            if (!copyOptions.editable.isInstance(target)) {
                throw new IllegalArgumentException(StringUtils.format("Target class [{}] not assignable to Editable class [{}]", target.getClass().getName(), copyOptions.editable.getName()));
            }
